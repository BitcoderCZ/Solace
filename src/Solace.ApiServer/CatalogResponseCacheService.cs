...
public sealed class CatalogResponseCacheService
{
    ...
    public ItemsCatalog GetItemsCatalog()
    {
        var lazy = _cache.GetOrCreate("Catalog_ItemsCatalog", entry =>
        {
            ...
            return new Lazy<ItemsCatalog>(CreateItemsCatalog);
        });
    ...
    public RecipesCatalog GetRecipeCatalog()
    {
        var lazy = _cache.GetOrCreate("Catalog_RecipeCatalog", entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(15);

            return new Lazy<RecipesCatalog>(CreateRecipeCatalog);
        });
    ...
    public JournalCatalog GetJournalCatalog()
    {
        var lazy = _cache.GetOrCreate("Catalog_JournalCatalog", entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(15);

            return new Lazy<JournalCatalog>(CreateJournalCatalog);
        });
    ...
    private ItemsCatalog CreateItemsCatalog()
    {
        ...
        foreach (Catalog.ItemEfficiencyCategoriesCatalogR.EfficiencyCategory efficiencyCategory in _catalog.ItemEfficiencyCategoriesCatalog.EfficiencyCategories)
        {
    ...
    private RecipesCatalog CreateRecipeCatalog()
    {
        ...
        RecipesCatalog.SmeltingRecipe[] smelting = [.. _catalog.RecipesCatalog.Smelting.Select(recipe =>
        {

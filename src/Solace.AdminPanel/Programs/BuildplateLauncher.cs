...
#pragma warning restore IDE0060 // Remove unused parameter
    ...
    public static Process? Run(Settings settings, ILogger logger)
    {
        ...
        {
            ProgramsLogs.LogCouldNotFindVersionedFile(logger, "buildplate connector plugin jar", Path.Combine(serverJarsDir, "buildplate-connector-plugin-{{version}}-SNAPSHOT-jar-with-dependencies.jar"), MinimumBuildplateConnectorPluginVersion);
            return null;
        }
        ...
        {
            ProgramsLogs.LogCouldNotFindVersionedFile(logger, "fountain bridge jar", Path.Combine(serverJarsDir, "fountain-{{version}}-SNAPSHOT-jar-with-dependencies.jar"), MinimumFountainBridgeVersion);
            return null;
        }
